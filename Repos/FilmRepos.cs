...
{
    public class FilmRepos: IFilmRepos
    {
        ...
        public async Task<List<Film>> ListeFilm(){
              try {
        ...
        public async Task<Film> SaveFilm(DtoAuteur dtoObjet)
        {

           
            Film dalObjet = new Film();
            DateTime dateoperation = DateTime.Now;

            try {

              dalObjet = _mapper.Map<Film>(dtoObjet) ;
            dalObjet.Code =new Guid().ToString();
                 dalObjet.DateCreation = dateoperation;
                 dalObjet.ModifierPar = dtoObjet.Userid;

             await _context.Films.AddAsync(dalObjet);
